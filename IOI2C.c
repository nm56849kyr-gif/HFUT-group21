...
unsigned char I2C_ReadOneByte(unsigned char I2C_Addr,unsigned char addr)
{
	...
	IIC_Start();	
	IIC_Send_Byte(I2C_Addr);	   //发送写命令 Send a write command
	res++;
	IIC_Wait_Ack();
...
u8 IICreadBytes(u8 dev, u8 reg, u8 length, u8 *data){
	...
	IIC_Start();
	IIC_Send_Byte(dev);	   //发送写命令 Send a write command
	IIC_Wait_Ack();
	IIC_Send_Byte(reg);   //发送地址 Send Address
  IIC_Wait_Ack();	  
	IIC_Start();
	IIC_Send_Byte(dev+1);  //进入接收模式	 Entering receive mode
	IIC_Wait_Ack();
	
    for(count=0;count<length;count++){
		 
		 if(count!=length-1)   data[count]=IIC_Read_Byte(1);  //带ACK的读数据 Read data with ACK
		 else                  data[count]=IIC_Read_Byte(0);  //最后一个字节NACK Last byte NACK
	}
    IIC_Stop();//产生一个停止条件  Generates a stop condition
    return count;
}
