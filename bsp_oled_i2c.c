...
{
    RCC->APB2ENR |= 1 << 3;   //先使能外设IO PORTB时钟 Enable peripheral IO PORTB clock first
 ...
 * @Note:
 * @Parm:
 * @Retval: 1:接收应答成功(Receive response successfully) | 0:接收应答失败(Failed to receive response)
 */
 ...
 * @Retval: 0:停止(stop) | 1:连续写(Continuous Write)
 */
...
unsigned char OLED_I2C_ReadOneByte(unsigned char I2C_Addr, unsigned char addr)
{
    ...
    OLED_IIC_Start();
    OLED_IIC_Send_Byte(I2C_Addr); //发送写命令 Send a write command
    res++;
    OLED_IIC_Wait_Ack();
 ...
 * @Note: 
 * @Parm: dev  目标设备地址(Target device address) | reg   寄存器地址(Register Address) | length 要读的字节数(The number of bytes to read) | *data  读出的数据将要存放的指针(Pointer to where the read data will be stored0
...
uint8_t OLED_OLED_IICreadBytes(uint8_t dev, uint8_t reg, uint8_t length, uint8_t *data)
{
    ...
    OLED_IIC_Start();
    OLED_IIC_Send_Byte(dev); //发送写命令 Send a write command
    OLED_IIC_Wait_Ack();
 ...
 * @Note: 
 * @Parm: dev  目标设备地址(Target device address) | reg   寄存器地址(Register Address) | length 要写的字节数(The number of bytes to write) | *data  将要写的数据的首地址(The first address of the data to be written)
